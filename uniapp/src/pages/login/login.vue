        ...
        <u-checkbox v-model="isCheckAgreement" shape="circle">
          <view class="text-xs flex">
            ...
            <view
              class="text-primary"
              hover-class="none"
              @click.stop="goPage('/bundle/pages/service_explan/index?type=1')"
            >
              《服务协议》
            </view>
            和
            <view
              class="text-primary"
              hover-class="none"
              @click.stop="goPage('/bundle/pages/service_explan/index?type=2')"
            >
              《隐私协议》
            </view>
          </view>
...
const includeLoginWay = (way: LoginWayEnum) => {
  return appStore.config.login_way?.includes(String(way));
};

const inWxAuth = computed(() => {
  return appStore.config.wechat_auth;
});

const isOpenAgreement = computed(() => appStore.config.login_agreement == 1);

const isOpenOtherAuth = computed(() => appStore.config.third_auth == 1);
